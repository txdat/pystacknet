...
def smape(y_true, y_pred, sample_weight=None):
    ...
    eps = 1e-15
    sape = (
        np.abs(y_true - y_pred) / (0.5 * (np.abs(y_true) + np.abs(y_pred)) + eps)
    ) * 100
...
def check_regression_metric(metric):
    ...
    if isinstance(metric, str):
        if metric not in valid_regression_metrics:
            raise Exception(
                "The regression metric has to be one of %s "
                % (", ".join([str(k) for k in valid_regression_metrics]))
            )
        if metric == "rmse":
            return rmse, metric
        elif metric == "mae":
        ...
        try:
            ...
            raise Exception(
                "The custom metric has to implement metric(y_true, y_pred, sample_weight=None)"
            )
...
def check_classification_metric(metric):
    ...
    if isinstance(metric, str):
        if metric not in valid_classification_metrics:
            raise Exception(
                "The classification metric has to be one of %s "
                % (", ".join([str(k) for k in valid_classification_metrics]))
            )
        if metric == "auc":
            return auc, metric
        elif metric == "logloss":
