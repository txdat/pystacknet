...
def _parallel_predict_proba(estimator, X, index):
    ...
    if hasattr(estimator, "predict_proba"):
        ...
        raise Exception(
            "Each model/algorithm needs to implement at least one of ('predict()','predict_proba()' or 'transform()' "
        )
    ...
    for estimator in estimators:
        ...
        if type(preds) is type(None):
            ...
            if predictions.shape != preds.shape:
                ...
                raise Exception(
                    " predictions' shape not equal among estimators within the  batch as %d!=%d "
                    % (predictions.shape[1], preds.shape[1])
                )
...
def _parallel_predict(estimator, X, index):
    ...
    if hasattr(estimator, "predict"):
        ...
        raise Exception(
            "Each model/algorithm needs to implement at least one of ('predict()' or 'transform()' "
        )
...
class StackNetClassifier(BaseEstimator, ClassifierMixin):
    def __init__(
        ...
        if type(models) is type(None):
            raise Exception(
                "Models cannot be None. It needs to be a list of sklearn type of models "
            )
        ...
        for l in range(len(models)):
            if not isinstance(models[l], list):
                raise Exception(
                    "Each element in the models' list has to be a list . In other words a 2-dimensional list is epected. "
                )
            for m in range(len(models[l])):
                if not hasattr(models[l][m], "fit"):
                    raise Exception(
                        "Each model/algorithm needs to implement a 'fit() method "
                    )

                if (
                    not hasattr(models[l][m], "predict_proba")
                    and not hasattr(models[l][m], "predict")
                    and not hasattr(models[l][m], "transform")
                ):
                    raise Exception(
                        "Each model/algorithm needs to implement at least one of ('predict()','predict_proba()' or 'transform()' "
                    )
        ...
        if restacking not in [True, False]:
            raise Exception(
                "restacking has to be True (to include previous inputs/layers to current layers in stacking) or False"
            )
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if len(classes) <= 1:
            raise Exception(
                "Number of classes must be at least 2, here only %d was given "
                % (len(classes))
            )
        ...
        for level in range(len(self.models)):
            ...
            if not type(previous_input) is type(None) and self.restacking:
                if self._sparse:
                    ...
                    current_input = csr_matrix(
                        hstack([csr_matrix(previous_input), csr_matrix(current_input)])
                    )
                else:
            ...
            if self.verbose > 0:
                print(
                    "Input Dimensionality %d at Level %d "
                    % (current_input.shape[1], level)
                )
            ...
            if self.verbose > 0:
                print(
                    "%d models included in Level %d " % (len(this_level_models), level)
                )
            ...
            for train_index, test_index in indices:
                ...
                if not type(sample_weight) is type(None):
                    w_train, w_cv = (
                        ...
                        sample_weight[test_index],
                    )

                all_results = Parallel(
                    n_jobs=min(self.n_jobs, len(this_level_models)), verbose=0
                )(
                    delayed(_parallel_build_estimators)(
                        clone(this_level_models[d]), X_train, y_train, w_train, d
                    )
                ...
                this_level_estimators_ = sorted(
                    this_level_estimators_, key=operator.itemgetter(1), reverse=False
                )
                ...
                all_results = Parallel(
                    ...
                    delayed(_parallel_predict_proba)(
                        this_level_estimators_[d][0], X_cv, d
                    )
                    for d in range(len(this_level_models))
                )
                this_level_predictions_ = [[t[0], t[1]] for t in all_results]

                this_level_predictions_ = sorted(
                    this_level_predictions_, key=operator.itemgetter(1), reverse=False
                )
                ...
                for d in range(len(this_level_models)):
                    ...
                    if self.use_proba:
                        if hasattr(this_model, "predict_proba"):
                            metrics_i[d] = self.metric(
                                y_cv, predictions_[d], sample_weight=w_cv
                            )
                            metrics[d] += metrics_i[d]
                            if self.verbose > 0:
                                print(
                                    "Fold %d/%d , model %d , %s===%f "
                                    % (
                                        i + 1,
                                        iter_count,
                                        d,
                        ...
                        if hasattr(this_model, "predict_proba"):
                            ...
                            metrics_i[d] = self.metric(
                                y_cv, preds_transformed, sample_weight=w_cv
                            )  #
                            metrics[d] += metrics_i[d]
                            if self.verbose > 0:
                                print(
                                    "Level %d, fold %d/%d , model %d , %s===%f "
                                    % (
                                        level,
                                        i + 1,
                                        iter_count,
                ...
                if type(train_oof) is type(None):
                    train_oof = np.zeros(
                        (current_input.shape[0], preds_concat_.shape[1])
                    )
                    self._level_dims.append(preds_concat_.shape[1])

                if self._level_dims[level] != preds_concat_.shape[1]:
                    raise Exception(
                        "Output dimensionality among folds is not consistent as %d!=%d "
                        % (self._level_dims[level], preds_concat_.shape[1])
                    )
                train_oof[test_index] = preds_concat_
                if self.verbose > 0:
                    print(
                        "=========== end of fold %i in level %d ==========="
                        % (i + 1, level)
                    )
                i += 1
            ...
            if self.verbose > 0:
                for d in range(len(this_level_models)):
                    ...
                    if hasattr(this_model, "predict_proba"):
                        print(
                            "Level %d, model %d , %s===%f "
                            % (level, d, self.metric_name, metrics[d])
                        )
            ...
            if self.use_retraining:
                ...
                all_results = Parallel(
                    ...
                    delayed(_parallel_build_estimators)(
                        clone(this_level_models[d]), current_input, y, sample_weight, d
                    )
                    for d in range(len(this_level_models))
                )
            ...
            if self.verbose > 0:
                print(
                    "Output dimensionality of level %d is %d "
                    % (level, current_input.shape[1])
                )
            ...
            if self.verbose > 0:
                ...
                print(
                    " level %d lasted %f seconds "
                    % (level, end_of_level_time - start_level_time)
                )
    ...
    def predict_proba(self, X):
        ...
        if type(self.n_classes_) is type(None) or self.n_classes_ == 1:
            raise Exception(
                "fit() must run successfuly to be able to execute the current method. "
            )
        ...
        if X.shape[1] != self.n_features_:
            raise Exception(
                "Input dimensionality of %d is not the same as the trained one with %d "
        ...
        for level in range(len(self.estimators_)):
            ...
            if self.verbose > 0:
                print(
                    "%d estimators included in Level %d "
                    % (len(this_level_estimators), level)
                )

            all_results = Parallel(
                n_jobs=min(self.n_jobs, len(this_level_estimators[0])), verbose=0
            )(
                delayed(_parallel_predict_proba_scoring)(
                    [
                        this_level_estimators[s][d]
                        for s in range(len(this_level_estimators))
                    ],
                    current_input,
                    d,
                )
            ...
            if test_pred.shape[1] != self._level_dims[level]:
                raise Exception(
                    "Output dimensionality for level %d with %d is not the same as the one during training with %d "
                    % (level, test_pred.shape[1], self._level_dims[level])
                )
                ...
                for d in range(len(this_level_models)):
                    ...
                    if hasattr(this_model, "predict"):
                        ...
                        if self.verbose > 0:
                            print(
                                "Fold %d/%d , model %d , %s===%f "
                                % (i + 1, iter_count, d, self.metric_name, metrics_i[d])
                            )
